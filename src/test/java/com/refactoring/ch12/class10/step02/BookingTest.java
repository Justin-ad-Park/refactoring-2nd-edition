...
class BookingTest {
    ...
    void hasTalkbackForPremium() {
        ...
        Booking booking = Booking.createPremiumBooking(show, date, extras);
        // when
        boolean result = booking.hasTalkback();
        // then
        Assertions.assertThat(result).isTrue();
    ...
    @DisplayName("동적으로 예약을 프리미엄 변경할 수 있다.")
    @Test
    void testCreateBooking() {
        ...
        // then
        Assertions.assertThat(booking.basePrice()).isEqualTo(10000);
        Assertions.assertThat(booking.hasTalkback()).isTrue();
        Assertions.assertThat(booking.hasDinner()).isFalse();

        // when
        Extra premiumFee = new Extra("premiumFee", 3000);
        Extra dinner = new Extra("dinner", 5000);
        booking.bePremium(List.of(premiumFee, dinner));

        // then
        Assertions.assertThat(booking.basePrice()).isEqualTo(18000);
        Assertions.assertThat(booking.hasDinner()).isTrue();
    }
